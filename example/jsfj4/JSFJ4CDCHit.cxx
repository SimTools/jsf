...
JSFJ4CDCHit::JSFJ4CDCHit(J4CDCDriftRegionHit *ahit)
{
  ...
  fSuperLayerNo=vc->GetMother()->GetMother()->GetMother()->GetMyID();

  fPDGParticleID = ahit->GetParticle()->GetPDGEncoding();   

  fCellID=ahit->GetCloneID();
  fTrackID=ahit->GetTrackID();
  fUniqueHitID=ahit->GetHitNumber();
...
void JSFJ4CDCHit::Print()
{
  ...
  cout << " EnergyDeposit(eV)=" << GetEnergyDeposit() ;
  cout << " Tof(nsec)=" << GetTof() ;
  cout << endl;
