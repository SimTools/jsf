...
CAkiyaMiyamoto        ELSEIF(K(IMTAU,2).EQ.K(ITAU,2)) THEN
C replaced with following two lines according to Tim
        ELSEIF(K(IMTAU,2).EQ.K(ITAU,2).AND.K(IMTAU,1).NE.3.AND.
     &         K(IMTAU,1).NE.13.AND.K(IMTAU,1).NE.14 ) THEN
...
CAkiya Miyamoto, Added to restore tau production vertex position, which is 
C destroied by PYROBO call.  2011-08-24
          DO 202 KK=1,5
            V(IP,KK)=VSAVE(KK)
202       CONTINUE
